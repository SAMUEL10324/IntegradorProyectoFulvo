...
if (!$id_reseña) {
    echo '<script>alert("ID de reseña no válido."); window.location = "5_gestion_reseña.php";</script>';
    exit();
}
...
if (!$id_cliente) {
    echo '<script>alert("No se pudo verificar tu identidad."); window.location = "5_gestion_reseña.php";</script>';
    exit();
}
...
if ($resultado && mysqli_affected_rows($conexion) > 0) {
    echo '<script>alert("Reseña eliminada correctamente."); window.location = "5_gestion_reseña.php";</script>';
} else {
    echo '<script>alert("No se pudo eliminar la reseña. Puede que no te pertenezca."); window.location = "5_gestion_reseña.php";</script>';
}
?>
